...
class Para:
    ...
    def run(self):
        ...
        thejob = job.load(loadsave.load_json(path.fullfile(self.jobconfig)))
        if self.runmode == 'FirstOnly':
            return self.run_seq(thejob, finalfolders[0:1])
        if self.runmode == 'Parallel':
            return self.run_para(thejob, finalfolders)
        if self.runmode == 'Sequential':
            return self.run_seq(thejob, finalfolders)
        else:
