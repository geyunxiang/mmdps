...
def streamline_tensors(streamline, evals_map=None):#evals=[0.001, 0, 0]):
    """
    ...
    for grad_idx, this_grad in enumerate(grad):
        eval = [evals_map[streamline[grad_idx, 0].astype('int'), streamline[grad_idx, 1].astype('int'), streamline[grad_idx, 2].astype('int')], 0, 0]
        #print(eval)
        tensors[grad_idx] = grad_tensor(this_grad, eval)
    return tensors


def streamline_signal(streamline, gtab, evals_map=None):#evals=[0.001, 0, 0]):
    """
    ...
    tensors = streamline_tensors(streamline, evals_map)
...
class LifeSignalMaker(object):
    ...
    def __init__(self, gtab, evals_map=None, sphere=None):
        """
    ...
    def calc_signal(self, xyz, vox_loc, evals):
        #vox_loc = self.sphere.find_closest(xyz)
        idx = self.sphere.find_closest(xyz)
        #_hash = idx* vals_index

        #if not self._calculated.has_key(vox_loc_hash):
        ...
        sig = sig - np.mean(sig)
            #self.signal[idx] = sig
        #self._calculated[vox_loc_hash] = sig

        return sig#self._calculated[vox_loc_hash]

    def streamline_signal(self, streamline):
        """
        Approximate the signal for a given streamline
        """
        #print(len(streamline.shape))
        grad = streamline_gradients(streamline)

        sig_out = np.zeros((grad.shape[0], self.signal.shape[-1]))
        for ii, g in enumerate(grad):
            vox_loc = [streamline[ii, 0].astype('int'), streamline[ii, 1].astype('int'), streamline[ii, 2].astype('int')]
            eval = [self.evals_map[vox_loc], 0, 0]

            sig_out[ii] = self.calc_signal(xyz=g, vox_loc=vox_loc, evals=eval)
            #print(g)
        return sig_out
...
class FiberModel(ReconstModel):
    ...
    def setup(self, streamline, affine, evals_map=None, sphere=None):
        """
        ...
        for s_idx, s in enumerate(streamline):
            if sphere is not False:
                ...
                fiber_signal.append(streamline_signal(s, self.gtab, evals_map=evals_map))
    ...
    def fit(self, data, streamline, affine, evals_map=None,
            sphere=None):
        """
        ...
        life_matrix, vox_coords = \
            self.setup(streamline, affine, evals_map=evals_map, sphere=sphere)
        ...
        return FiberFit(self, life_matrix, vox_coords, to_fit, beta,
                        ...
                        vox_data, streamline, affine, evals_map)
