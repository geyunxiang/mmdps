...
class SparseFascicleModel(ReconstModel, Cache):
    def __init__(self, gtab, sphere=None, evals_map=None, index_map=None,
        ...
        if sphere is None:
            sphere = dpd.get_sphere()
        self.sphere = sphere
        self.evals_map = np.asarray(evals_map)
        self.index_map = np.asarray(index_map)
    ...
    #@auto_attr
    def design_matrix(self, response):
        """
        ...
        return sfm_design_matrix(self.gtab, self.sphere, response,
                                 'signal')

    def fit(self, loc, data, mask=None):
        """
        ...
        ma = self.cache_get('ma', key=self.index_map[loc])


        if ma is None:
            ma = self.design_matrix(response=self.evals_map[loc])
            self.cache_set('ma', key=self.index_map[loc], value=ma)

        #print(self.evals_map[loc])
        ...
        #print('flat_S:', flat_S.shape)
        #print('flat_params:', flat_params.shape)
        #print('signal_matrix:', self.design_matrix.shape)
        ...
        for vox, vox_data in enumerate(flat_S):
            ...
            if not (np.any(~np.isfinite(vox_data)) or np.all(vox_data == 0)):
                ...
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore")
                    flat_params[vox] = self.solver.fit(ma,
                                                       fit_it).coef_
        ...
        return SparseFascicleFit(self, beta, S0, isotropic, loc)


class SparseFascicleFit(ReconstFit):
    def __init__(self, model, beta, S0, iso, loc):
        """
    ...
    def odf(self, sphere):
        ...
        odf_matrix = self.model.cache_get('odf_matrix', key=self.model.index_map[self.loc])

        #print('get cache!', time.time()-time0)
        ...
        if odf_matrix is None:
            ...
            odf_matrix = sfm_design_matrix(sphere, self.model.sphere,
                                           self.model.evals_map[self.loc], mode='odf')



            self.model.cache_set('odf_matrix', key=self.model.index_map[self.loc], value=odf_matrix)
            #print('calculating!', time.time()-time1)
            #time0=time.time()


        #print('odf_matrix:', odf_matrix.shape)
        #print('beta:', self.beta.shape)


        flat_beta = self.beta.reshape(-1, self.beta.shape[-1])

        #print('beta:', flat_beta.shape)


        flat_odf = np.dot(odf_matrix, flat_beta.T)

        #print('return results!', time.time() - time0)
